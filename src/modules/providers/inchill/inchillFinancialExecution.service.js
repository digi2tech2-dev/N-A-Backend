...
class InchillFinancialExecutionService {
    ...
    async execute(orderId) {
        ...
        // This is an authoritative post-send response proving the provider did
        // not attempt a mutation. It is therefore safe to settle only this
        // just-sent order as failed and refund it; timeout evidence remains
        // ambiguous and must continue to use the UNKNOWN path below.
        if (upstream === 'NOT_SENT' && !transaction.upstreamTimeout) return this._failedNotSent(orderId, 'NOT_SENT', evidence, [STATES.SENT]);
        ...
        return this._unknown(orderId, transaction.upstreamTimeout ? 'TIMEOUT' : 'UNKNOWN', evidence);
    }
    async _failedNotSent(orderId, reason, evidence = {}, allowedStates = [STATES.READY, STATES.CLAIMED]) { const failed = await this.Order.findOneAndUpdate({ _id: orderId, 'inchillFinancial.mutationState': { $in: allowedStates } }, { $set: { status: ORDER_STATUS.FAILED, providerStatus: 'NOT_SENT', failedAt: this.now(), 'inchillFinancial.mutationState': STATES.FAILED, ...evidence, 'inchillFinancial.providerStatus': 'NOT_SENT', 'inchillFinancial.unknownReason': String(reason), 'inchillFinancial.outcomeAt': this.now() } }, { new: true }); if (failed && this.refundFailedOrder) await this.refundFailedOrder(failed); return { handled: true, order: failed, placed: false, refunded: Boolean(failed) }; }
